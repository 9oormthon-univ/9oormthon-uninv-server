...
import { MemberOverviewDto, ReadMemberOverviewResponseDto } from '../dto/response/read-member-overview.response.dto';

@Injectable()
@UseFilters(HttpExceptionFilter)
export class ReadMemberOverviewService {
  constructor(
  ...
  async execute(adminId: number, teamId: number) {
    return this.dataSource.transaction(async (manager) => {
      ...
      const members: MemberOverviewDto[] = team.members.map(
        (member) => {
          return {
            id: member.id,
